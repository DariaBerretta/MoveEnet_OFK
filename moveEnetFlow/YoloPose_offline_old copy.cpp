...
public:
    bool init(const std::string &model_path,
          ...
          double network_period,
          const cv::Size &image_size,
          const std::string &device = std::string())
    {
        sensor_size = image_size;

        if (network_period <= 0.0) {
            yError() << "network_period must be greater than zero. Received:"
                    << network_period;
            return false;
        }
        ...
        next_inference_ts = 0.0;
        previous_frame_ts = -1.0;
        schedule_initialized = false;

        inference_count = 0;
        first_inference_ts = -1.0;
        last_inference_ts = -1.0;

        yInfo() << "Requested YOLO inference period:" << period << "s";
        yInfo() << "Requested YOLO inference rate:" << 1.0 / period << "Hz";
...
//         Bottle *mn_container = input_port.read(true);  // blocking wait
//         if (!mn_container) {
//             return false;
//         }

//         detected_skeleton.pose = hpecore::extractSkeletonFromYARP<Bottle>(*mn_container);
//         detected_skeleton.conf = hpecore::extractConfidenceFromYARP<Bottle>(*mn_container);

//         // This is the timestamp of the video frame that produced the pose.
//         detected_skeleton.timestamp = frame_ts;

//         // This is real processing latency, not video-time offset.
...
bool update(const cv::Mat &latest_image,
            ...
            hpecore::stampedPose &detected_skeleton)
    {
        ...
        // Se il timestamp torna indietro, il video è stato riavviato
        // oppure è iniziata una nuova sequenza.
        if (schedule_initialized &&
            frame_ts + time_epsilon < previous_frame_ts) {
            schedule_initialized = false;
        }

        previous_frame_ts = frame_ts;

        // La prima inferenza viene eseguita sul primo frame disponibile.
        if (!schedule_initialized) {
            next_inference_ts = frame_ts;
            schedule_initialized = true;
        }

        // Il frame corrente precede il prossimo istante programmato:
        // non deve essere inviato a YOLO.
        if (frame_ts + time_epsilon < next_inference_ts) {
            return false;
        }

        /*
        * Avanza il calendario delle inferenze.
        *
        * Usiamo un calendario assoluto:
        *     next += period
        *
        * invece di:
        *     last_frame_used = frame_ts
        *
        * Questo evita una deriva temporale quando il periodo richiesto
        * non è un multiplo esatto del frame period del video.
        */
        do {
            next_inference_ts += period;
        } while (next_inference_ts <= frame_ts + time_epsilon);

        // ===== Questo frame è stato selezionato per YOLO =====
        ...
        // Invia un solo frame al server Python.
        output_port.write();

        // Aspetta il risultato relativo al frame appena inviato.
        ...
        // Timestamp del frame video usato per questa inferenza.
        detected_skeleton.timestamp = frame_ts;

        // Tempo reale richiesto dall'elaborazione YOLO.
        detected_skeleton.delay =
            yarp::os::Time::now() - wall_start;

        // Conta tutte le inferenze completate, anche quelle che producono
        // una posa nulla.
        if (inference_count == 0) {
            first_inference_ts = frame_ts;
        }

        last_inference_ts = frame_ts;
        inference_count++;

        return hpecore::poseNonZero(detected_skeleton.pose);
    }

    std::size_t getInferenceCount() const
    {
        return inference_count;
    }

    double getFirstInferenceTimestamp() const
    {
        return first_inference_ts;
    }

    double getLastInferenceTimestamp() const
    {
        return last_inference_ts;
    }
...
int main(int argc, char *argv[]){
    ...
    std::string yolo_model_path = rf.check("yolo_model_path", Value("/workspace/model_mounts/YoloPose/yolo26n-pose.pt")).asString();
    std::string yolo_script_path = rf.check("YoloPose_script", Value("/workspace/model_mounts/YoloPose/YoloPose_yarp_server.py")).asString();
    std::string powerjoular_file = rf.check("pwrjlr_file", Value("")).asString();
    std::string gpu_monitor_file = rf.check("gpu_file", Value("/workspace/moveEnetFlow/pwr_gpu_file/single_test/YoloPose_gpu_pwr")).asString();
    ...
    if (!yolo_handler.init(yolo_model_path, yolo_script_path, net_period, res, device)) {
    ...
    while (true) {
        ...
        tnow = cap.get(cv::CAP_PROP_POS_MSEC) / 1000.0; // current timestamp in seconds
        ...
        if (csv_file.is_open()) {
            ...
            row << std::fixed << std::setprecision(6) << detected_pose.timestamp;
            row << "," << detected_pose.delay;
