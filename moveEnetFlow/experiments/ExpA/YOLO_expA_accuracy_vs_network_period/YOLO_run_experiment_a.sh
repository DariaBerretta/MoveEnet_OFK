...
# Network periods in seconds: 50 Hz, 20 Hz, 10 Hz, 5 Hz, 2 Hz
PERIODS=("0.02" "0.05" "0.1" "0.2" "0.5")

# Core executable and data paths
BINARY="/workspace/moveEnetFlow/build2/YoloPose_offline"              # OpenPose-only binary
...
OUTPUT_PERIOD="0.02"                                               # CSV output sampling period, dataset at maxmimum 50Hz
...
Options:
  --binary <path>            Path to YoloPose_offline binary
...
if awk "BEGIN{exit !($OUTPUT_PERIOD >= 0.02)}"; then
  ...
  echo "YoloPose_offline cannot log faster than 0.02s per row; clamping output_period to 0.02" >&2
  OUTPUT_PERIOD="0.02"
fi
...
if [[ "$dev_l" == cpu* || "$dev_l" == "cpu" ]]; then
  echo "GPU required for OpenPose_offline. Device set to CPU: $DEVICE" >&2
  exit 1
fi
...
for NP in "${PERIODS[@]}"; do
  ...
  for MP4_FILE in "${MP4_FILES[@]}"; do
    ...
    OUT_CSV="$NP_DIR/${REL_STEM}_yolopose_np_${SAFE_NP}.csv"
    LOG_FILE="$MP4_DIR/${REL_STEM}_yolopose_np_${SAFE_NP}.log"
    ...
    echo "  -> [$REL_STEM] YoloPose, NP=$NP"
    if ! "$BINARY" "${COMMON_ARGS[@]}" --output_csv "$OUT_CSV" > "$LOG_FILE" 2>&1; then
      echo "FAILED: NP=$NP, file=$MP4_FILE"
