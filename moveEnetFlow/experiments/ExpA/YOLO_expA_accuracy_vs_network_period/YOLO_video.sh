...
# Save a synchronized YOLO Pose video for one eH36M RGB sequence.
#
# Final video contents:
#   - eH36M RGB frames
#   - eH36M ground-truth skeleton in green
#   - YOLO Pose prediction in magenta
#
# YOLO inference and video/CSV output use independent rates:
#   - NET_PERIOD controls how often YOLO is executed.
#   - OUTPUT_PERIOD controls the held-pose CSV/video rate.
#
# With the defaults:
#   - YOLO runs at 50 Hz       (NET_PERIOD=0.02 s)
#   - output is produced at 200 Hz (OUTPUT_PERIOD=0.005 s)
#
# Between two YOLO inferences, YoloPose_offline keeps the latest valid pose.
...
YOLO_MODEL_PATH="/workspace/model_mounts/YoloPose/yolo26n-pose.pt"
...
YOLO Pose:
  --binary <path>             YoloPose_offline executable
  --yolo_model_path <path>    Ultralytics pose model (.pt)
  --yolo_script <path>        YoloPose_yarp_server.py
  --net_period <seconds>      Interval between YOLO inferences. Default: 0.02
  --output_period <seconds>   Held-pose CSV/video period. Default: 0.005
  --device <device>           Ultralytics device. Default: cuda:0
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --yolo_model_path) YOLO_MODEL_PATH="$2"; shift 2 ;;
...
if [[ ! -f "$YOLO_SCRIPT" ]]; then
  echo "YOLO Pose sidecar not found: $YOLO_SCRIPT" >&2
  exit 1
fi
...
# The binary requires an already configured YARP name server.
if command -v yarp >/dev/null 2>&1; then
  if ! yarp where >/dev/null 2>&1; then
    echo "YARP name server is not available. Start yarpserver, then rerun this script." >&2
    exit 1
  fi
fi
...
STEM="${SEQUENCE}_yolopose_np_${SAFE_NET_PERIOD}_op_${SAFE_OUTPUT_PERIOD}"
...
echo "YOLO Pose synchronized video"
...
echo "Overlaying eH36M GT and redrawing YOLO Pose prediction..."
...
while frame_index < len(timestamps):
    ...
    # GT is drawn first, prediction second, so YOLO remains visible where the
