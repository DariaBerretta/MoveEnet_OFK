...
#   2. a prediction-only MP4 reconstructed from the same independent
#      net-period windows used by PointNet;
...
# is rendered directly in the hpe-core data.log/OpenCV convention:
#     x_view = x_log
#     y_view = y_log
# The EventPointPose coordinate conversion remains internal to the server.

BINARY="/workspace/moveEnetFlow/build2/eventPointPose_offline_no_rolling"
...
OUTPUT_DIR="$(pwd)/epp_single_entry_video_no_rolling"
NET_PERIOD="0.2"
OUTPUT_PERIOD="0.02"
DEVICE="cuda:0"
IMG_W="346"
...
EventPointPose:
  ...
  --net_period <seconds>          Independent event-batch interval. Default: $NET_PERIOD
  ...
  --events_per_window <int>       Required events in one net-period; fixed at 7500
...
Examples:
  ...
  $(basename "$0") --data_file /data/dhp19_testing_set_S13toS17/S13_1_1/ch3dvs/data.log --net_period 0.4
  $(basename "$0") --data_file /data/.../S13_1_1/ch2dvs/data.log --net_period 1.0 --max_packets 5000 --prediction_only
USAGE
}
...
if [[ "$FIFO_SIZE" != "7500" ]]; then
  echo "--events_per_window must be 7500 for the current net-period window." >&2
  exit 1
fi
...
echo "Period window     : latest $FIFO_SIZE events / $NUM_POINTS RasEPC points"
echo "Carryover         : disabled; each net-period is independent"
echo "Seed              : $SEED"
...
if [[ "$PRED_ROWS" -le 0 ]]; then
  ...
  echo "No complete net-period contained at least $FIFO_SIZE events." >&2
  echo "Increase --net_period for this current-period-only policy." >&2
  tail -n 40 "$RUN_LOG" >&2 || true
  exit 1
fi
...
if [[ "$GENERATE_VIDEO" == "true" ]]; then
  echo
  echo "Reconstructing EventPointPose net-period video..."
...
def render_period_window(window: np.ndarray) -> np.ndarray:
    # The video displays hpe-core data.log/OpenCV coordinates directly.
    # The y flip required by EventPointPose is applied only inside the server.
    x = window[:, 0]
    y = window[:, 1]
    polarity = window[:, 2]
...
# Reconstruct the independent net-period windows used by the new C++/Python
# pipeline. A period produces at most one model input: the latest 7500 valid
# events from that period. Incomplete periods are discarded and never combined
# with a later period.
successful_deadlines = []
successful_windows = []
period_start_event = 0
period_end_event = 0
next_inference_ts = float(packet_ts[0]) + net_period


def finalize_period(period_end_ts: float) -> None:
    global period_start_event, period_end_event
    if period_end_event <= period_start_event:
        return

    batch_x = event_x[period_start_event:period_end_event]
    batch_y = event_y[period_start_event:period_end_event]
    batch_p = event_p[period_start_event:period_end_event]
    batch_t = event_ts[period_start_event:period_end_event]
    valid = (
    ...
    if accepted.shape[0] >= fifo_size:
        successful_deadlines.append(float(period_end_ts))
        successful_windows.append(accepted[-fifo_size:, :].copy())


for packet_index, current_ts in enumerate(packet_ts):
    current_ts = float(current_ts)

    # Finalize deadlines strictly before this packet without including it.
    while current_ts > next_inference_ts + EPS:
        finalize_period(next_inference_ts)
        period_start_event = period_end_event
        next_inference_ts += net_period

    # Add the complete packet group. Exact-boundary packets belong to the
    # period ending at that boundary, matching EventPointPose_offline.cpp.
    period_end_event = int(ends[packet_index])
    if current_ts >= next_inference_ts - EPS:
        finalize_period(next_inference_ts)
        period_start_event = period_end_event
        next_inference_ts += net_period
...
if not successful_windows:
    raise SystemExit(
        "No reconstructed net-period contained at least "
        f"{fifo_size} valid events; increase --net_period"
    )

successful_deadlines_arr = np.asarray(successful_deadlines, dtype=np.float64)

try:
    for row_index, output_ts in enumerate(pred_ts):
        window_index = int(
            np.searchsorted(
                successful_deadlines_arr, float(output_ts) + EPS, side="right"
            ) - 1
        )
        if window_index < 0:
            raise SystemExit(
                f"Prediction row {row_index} at t={output_ts:.6f} precedes "
                "the first reconstructed valid inference"
            )

        event_frame = render_period_window(successful_windows[window_index])
...
print(f"Reconstructed valid net-period windows: {len(successful_windows)}")
