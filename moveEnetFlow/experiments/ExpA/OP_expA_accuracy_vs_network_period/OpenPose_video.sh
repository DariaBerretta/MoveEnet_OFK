...
# Save a synchronized OpenPose video for one eH36M RGB sequence.
#
...
OPENPOSE_MODEL_PATH="/usr/local/src/openpose/models/"
...
OpenPose:
  --binary <path>             OpenPose_offline executable
  --op_model_path <path>      OpenPose models directory
  --net_period <seconds>      Interval between OpenPose inferences. Default: 0.02
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --op_model_path) OPENPOSE_MODEL_PATH="$2"; shift 2 ;;
...
if [[ ! -d "$OPENPOSE_MODEL_PATH" ]]; then
  echo "OpenPose models directory not found: $OPENPOSE_MODEL_PATH" >&2
  exit 1
fi
...
STEM="${SEQUENCE}_openpose_np_${SAFE_NET_PERIOD}_op_${SAFE_OUTPUT_PERIOD}"
...
COMMON_ARGS=(
  --data_file "$DATA_FILE"
  --op_model_path "$OPENPOSE_MODEL_PATH"
...
echo "OpenPose synchronized video"
echo "RGB video      : $DATA_FILE"
echo "GT file        : $GT_FILE"
echo "OpenPose models  : $OPENPOSE_MODEL_PATH"
...
echo "Overlaying eH36M GT and redrawing OpenPose prediction..."
...
while frame_index < len(timestamps):
    ...
    # GT is drawn first, prediction second, so OpenPose remains visible where the
