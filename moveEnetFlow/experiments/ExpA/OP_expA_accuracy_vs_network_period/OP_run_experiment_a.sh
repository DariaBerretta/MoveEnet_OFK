...
BINARY="/workspace/moveEnetFlow/build2/OpenPose_offline"              # OpenPose-only binary
...
Options:
  --binary <path>            Path to OpenPose_offline binary
...
if awk "BEGIN{exit !($OUTPUT_PERIOD >= 0.02)}"; then
  ...
  echo "OpenPose_offline cannot log faster than 0.02s per row; clamping output_period to 0.02" >&2
  OUTPUT_PERIOD="0.02"
fi
...
for NP in "${PERIODS[@]}"; do
  ...
  for MP4_FILE in "${MP4_FILES[@]}"; do
    ...
    OUT_CSV="$NP_DIR/${REL_STEM}_openpose_np_${SAFE_NP}.csv"
    LOG_FILE="$MP4_DIR/${REL_STEM}_openpose_np_${SAFE_NP}.log"
